...
def identify_masks(src, dst, model_name, channels, diameter, flow_threshold=30, cellprob_threshold=1, figuresize=25, cmap='inferno', verbose=False, plot=False, save=False, custom_model=None, signal_thresholds=1000, normalize=True, resize=False, target_height=None, target_width=None, rescale=True, resample=True, net_avg=False, invert=False, circular=False, percentiles=None, overlay=True, grayscale=False):
    ...
    if custom_model != None:
	# Create the model with the custom pretrained model
        ...
        #print("Loaded keys from state dict:", model_state.keys())
    ...
    time_ls = []
    if normalize:
    	images, _, image_names, _ = load_normalized_images_and_labels(image_dir=src, label_dir=None, secondary_image_dir=None, signal_thresholds=signal_thresholds, channels=channels, percentiles=percentiles,  circular=circular, invert=invert, visualize=verbose)
    	images = [np.squeeze(img) if img.shape[-1] == 1 else img for img in images]
    	orig_dims = [(image.shape[1], image.shape[0]) for image in images]
    else:
        images, _, image_names, _ = load_images_and_labels(src, None, circular, invert) 
        images = [np.squeeze(img) if img.shape[-1] == 1 else img for img in images]
        orig_dims = [(image.shape[1], image.shape[0]) for image in images]
    if resize:
        images, _ = resize_images_and_labels(images, None, target_height, target_width, True)
        
    #diameter = model.estimate_diameter(image)
...
def load_images_and_labels(image_dir, label_dir, circular=False, invert=False, image_extension="*.tif", label_extension="*.tif"):
    images = []
    labels = []
...
def load_normalized_images_and_labels(image_dir, label_dir, secondary_image_dir=None, image_extension="*.tif", label_extension="*.tif", signal_thresholds=[1000], channels=None, percentiles=None,  circular=False, invert=False, visualize=False):
...
def generate_cp_masks(settings):
    ...
    identify_masks(src, dst, model_name, channels, diameter, flow_threshold, cellprob_threshold, figuresize, cmap, verbose, plot, save, custom_model, signal_thresholds, normalize, resize, target_height, target_width, rescale, resample, net_avg, invert, circular, percentiles, overlay, grayscale)
